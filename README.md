Refleksi Modul 4
1. Test Driven Development (TDD) sangat berguna dalam mengembangkan model dan fungsionalitas dalam sebuah sistem. Dengan menyiapkan berbagai skenario pengujian—baik untuk kasus yang berjalan sesuai harapan (happy path) maupun yang berpotensi mengalami kendala (unhappy path)—TDD membantu dalam mengantisipasi bug dan memastikan setiap bagian kode berfungsi sebagaimana mestinya. Selain itu, pendekatan ini mendorong penulisan kode yang lebih modular dan reusable, di mana fungsi atau metode yang kompleks dapat dipecah menjadi bagian-bagian kecil yang dapat digunakan kembali dalam konteks lain dengan algoritma serupa. TDD juga mempermudah proses refactoring karena pengujian yang telah dibuat akan memberikan indikasi jika ada perubahan yang mempengaruhi fungsionalitas tertentu, sehingga memudahkan identifikasi dan perbaikan kesalahan. Terakhir, pengujian dalam TDD berperan sebagai dokumentasi aktif, yang berarti bahwa pengujian itu sendiri sudah menjadi referensi mengenai cara kerja kode yang dikembangkan. Ini tidak hanya membantu saya sebagai pengembang tetapi juga memudahkan pengembang lain dalam memahami sistem yang telah dibuat.
2. Penerapan FIRST pada modul ini:
- Fast → Pengujian yang saya buat dapat dijalankan dengan cepat tanpa mengganggu workflow. Hal ini terlihat dari manajemen CI/CD yang telah memisahkan antara unit test dan functional test, penggunaan test double (Mock) untuk memverifikasi interaksi dengan objek, serta pemberian delay minimal dalam functional test yang telah terisolasi
- Isolated → Setiap test case bersifat independen dan tidak saling mempengaruhi. Ini dicapai melalui penggunaan test double (Mock) untuk memanggil fungsi dari class lain saat diperlukan, serta penerapan metode setUp untuk menyiapkan dummy dan mock object sebelum menjalankan setiap pengujian.
- Repeatable → Pengujian dapat dijalankan berulang kali dengan hasil yang konsisten tanpa perlu mengubah kode. Hal ini dimungkinkan dengan implementasi test double yang memastikan independensi setiap pengujian.
- Self-Validating → Pengujian menggunakan assertion yang ketat dan tepat untuk memastikan validitas hasil, bukan sekadar mencetak output. Saya memastikan setiap test memiliki cakupan assertion sekecil mungkin dengan jumlah minimal, meskipun dalam beberapa kasus, beberapa assertion digunakan untuk memastikan skenario lebih kompleks.
- Thorough/Timely → Pengujian mencakup berbagai kemungkinan skenario yang dapat terjadi pada fitur atau model yang dikembangkan. Ini mencakup pengujian untuk happy path dan unhappy path, yang telah diterapkan baik dalam tutorial ini maupun dalam kode sebelumnya untuk memastikan setiap kemungkinan kasus diuji dengan baik.
